...
// Force 256 prescaler for Slow (approx 328 kHz)
...
)
{
	...
	// --- STEP 2: PUT CARD IN IDLE STATE (CMD0) ---
	SPI_Timer_On(500);
	while (SPI_Timer_Status() && (res = send_cmd(CMD0, 0)) != 1) {
		/* Poll CMD0 until it returns 0x01 (Idle State) */
	}

	if (res == 1) {			/* Card entered Idle state */
		SPI_Timer_On(1000);					/* Initialization timeout = 1 sec */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);	/* Get R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* VCC 2.7-3.6V? */
				while (SPI_Timer_Status() && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for ACMD41(HCS) */
				if (SPI_Timer_Status() && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);
					ty = (ocr[0] & 0x40) ? CT_SD2 | CT_BLOCK : CT_SD2;	/* SDHC or SDv2 */
				}
			}
		} else {	/* SDv1 or MMC */
			...
			while (SPI_Timer_Status() && send_cmd(cmd, 0)) ;		/* Wait for init */
