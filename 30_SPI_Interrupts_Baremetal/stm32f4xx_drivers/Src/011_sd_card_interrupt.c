...
uint8_t SD_Init(void)
{
    ...
    // read R7 (IMPORTANT: CS still LOW)
    uint8_t r7[4];
    for(int i = 0; i < 4; i++)
...
uint8_t SD_ReadBlock(uint32_t blockAddr, uint8_t *buffer)
{
    ...
    SD_CS_LOW();
       ...
       if(SD_SendCommand(17, blockAddr, 0x01) != 0x00)
       {
           SD_CS_HIGH();
           return 1;
       }
