...
typedef struct
{
	...
	__vo uint32_t OTYPER;			/*GPIO port output type register					offset address: 0x04*/
	__vo uint32_t OSPEEDR;			/*GPIO port output speed register					offset address: 0x08*/
	__vo uint32_t PUPDR;	     	/*GPIO port pull-up/pull-down register  			offset address: 0x0C*/
	...
	__vo uint32_t AFR[2];			/* AFR[0]: GPIO alternate function low register		offset address: 0x20
	 	 	 	 	 	 	   	   	   AFR[1]: GPIO alternate function HIGH register	 offset address: 0x24*/
}GPIO_RegDef_t;
...
typedef struct
{
	...
	__vo uint32_t CSR;			/* RCC clock control & status register  							Address offset: 0x74 */
	 	 uint32_t RESERVED6[2];	/* Reserved										0x87 - 0x7C */
...
typedef struct
{
	__vo uint32_t IMR;			/*GPIO port mode register							offset address: 0x00*/
	__vo uint32_t EMR;			/*GPIO port output type register					offset address: 0x04*/
	__vo uint32_t RTSR;			/*GPIO port output speed register					offset address: 0x08*/
	__vo uint32_t FTSR;	     	/*GPIO port pull-up/pull-down register  			offset address: 0x0C*/
	__vo uint32_t SWIER;	    /*GPIO port input data register						offset address: 0x10*/
	__vo uint32_t PR;		   	/*GPIO port output data register					offset address: 0x14*/
...
typedef struct
{
	__vo uint32_t MEMRMP;			/*GPIO port mode register							offset address: 0x00*/
	__vo uint32_t PMC;				/*GPIO port output type register					offset address: 0x04*/
	__vo uint32_t EXTICR[4];		/*GPIO port output speed register					offset address: 0x08-0x14*/
	     uint32_t RESERVED1[2];	    /*GPIO port pull-up/pull-down register  			offset address: 0x18-0x1C*/
	__vo uint32_t CMPCR;	    	/*GPIO port input data register						offset address: 0x20*/
	     uint32_t RESERVED2[2];	    /*GPIO port pull-up/pull-down register  			offset address: 0x24-0x28*/
	__vo uint32_t CFGR;		   		/*GPIO port output data register					offset address: 0x2C*/
...
typedef struct
{
    ...
    volatile uint32_t CCR1;   // ⭐ ADD THIS
    volatile uint32_t CCR2;   // ⭐ ADD THIS
    volatile uint32_t CCR3;   // ⭐ ADD THIS
    volatile uint32_t CCR4;   // ⭐ ADD THIS
...
#define TIM2_PCLK_DI()		( RCC -> AHB1ENR &= ~( 1 << 0) )
#define TIM3_PCLK_DI()		( RCC -> AHB1ENR &= ~( 1 << 1) )
#define TIM4_PCLK_DI()		( RCC -> AHB1ENR &= ~( 1 << 2) )
#define TIM5_PCLK_DI()		( RCC -> AHB1ENR &= ~( 1 << 3) )
/*
...
/*
 * macros for all the possible priorities
 */
...
/**********************************
 * BIT POSITION DEFINATIONS OF I2C PERIPHERAL
 **********************************************/
...
#define TIM_CR1_CMS        5   // 5–6 (2 bits)
#define TIM_CR1_ARPE       7
#define TIM_CR1_CKD        8   // 8–9 (2 bits)

/*
 * TIMx_DIER bit positions
 */
...
/*
 * TIMx_CCMR1 bit positions
 */
#define TIM_CCMR1_CC1S     0   // 0–1
#define TIM_CCMR1_OC1FE    2
#define TIM_CCMR1_OC1PE    3
#define TIM_CCMR1_OC1M     4   // 4–6 (PWM mode here)

#define TIM_CCMR1_CC2S     8   // 8–9
#define TIM_CCMR1_OC2FE    10
#define TIM_CCMR1_OC2PE    11
#define TIM_CCMR1_OC2M     12  // 12–14

/*
 * TIMx_CCER bit positions
 */
...
#define TIM_PWM_MODE1      6   // 110
#define TIM_PWM_MODE2      7   // 111
